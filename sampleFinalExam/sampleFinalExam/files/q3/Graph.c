...
Connects *wellConnected(Graph g, int *nconns)
{
   ...
   Connects *new = calloc (g->nV, sizeof(new));
   ...
   *nconns = 0;
   // int numWC = 0;
   for (Vertex v = 0; validV(g, v) && v < g->nV; v++) {
      for (Vertex w = 0; validV(g, w) && w < g->nV; w++) {
         if (g->edges[v][w]) {
            new[v].nconn++;
         }
      }
   ...
   for (int i = 0; i < *nconns; i++) {
      ...
      for (int j = 0; j < g->nV; j++) {
         if (new[j].nconn >= 2 && new[j].nconn < new[prev].nconn 
               && new[j].nconn > new[curr].nconn) {
            curr = j;
         }
      }
      // nconns[i] = curr;
