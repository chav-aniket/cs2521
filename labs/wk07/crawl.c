...
int main (int argc, char **argv)
{
	...
	while (!emptyQueue(toDo) && nVertices(graph) < maxURLs) {
		...
		while (!url_feof (handle)) {
			...
			while ((pos = GetNextURL (buffer, baseURL, result, pos)) > 0) {
				printf ("Found: '%s'\n", result);
				if (nVertices(graph) < maxURLs || ) { //check if both vertices are added
